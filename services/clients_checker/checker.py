...
from data.file_extensions import is_valid_passport, yellow_fill, is_valid_pinfl, \
    contains_prohibited_product, red_fill, replace_words, highlight_invalid_cell
...
async def checker_info_from_wb(df, context, message):
    ...
    for index, row in df.iterrows():
        ...
        if is_valid == 0:
        ...
        elif is_valid is False or not is_valid_pinfl(pinfl):
            ...
            if check_in_service == 1:
                ...
                await message.answer(
                    f'Сервис по определению ПД не смог обработать passport = {cleaned_passport}  - pinfl = {pinfl} !,'
                    f'\n обратитесь к Админу @Razzakov_Timur'
                )
            else:
                highlight_invalid_cell(sheet, row_index, pinfl_col_idx, yellow_fill)
                invalid_data = True
